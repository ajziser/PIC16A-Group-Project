...
Python packages used (and versions): pandas 1.5.2, matplotlib 3.6.2, sklearn 0.20, numpy 1.23.5.
...
4. Paste and run the first 7 lines of the Demo File into a cell in the .ipynb file to import pandas, read the penguins data, and import the CurrentCode file. There should be no errors or results from running this cell. 
5. Run line 8 inside the same cell to get a preview of the existing penguins data.  You should see the dataframe like this:
...
6. Create a new cell and run lines 10-12 of the Demo File to create a Project class object with the penguins dataframe, and run the clean_data method. Line 12 will show the preview of the cleaned up data, which has fewer columns compared to the original data we saw in the previous step:
...
7. In a new cell, run line 14 of the demo file. This will run the scatterplot method of the class to compare the penguins Species with the Delta 15 N. You should see a graph like this:

### Result of running scatterplot method
...
8. In a new cell, run line 15 of the demo file. This will call the cluster function of the class to plot the two variables in the data against one another, colored.grouped by penguin species. You should see a graph like this:

### Result of running cluster method
...
9. Finally, run line 17 of the demo file code to create a decision tree with penguins data. It will print two values, the training data score (~ .939) and the test data score (~ .882) for the model. This model uses a max depth of 2 to avoid overfitting or underfitting, but you can modify the max depth or train-test split in the CurrentCode file to see how the accuracy scores change with a different kinds of decision trees. 
